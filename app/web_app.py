...
from main import Store, blank_lesson, uid, now, html_doc, write_docx
...
class Api:
    ...
    def save_lesson(self, lesson):
        lesson = json.loads(json.dumps(lesson, ensure_ascii=False))
    ...
    def export(self, kind, lesson):
        ...
        try:
            if kind == 'pdf':
                ...
                HTML(string=html_doc(lesson), base_url=str(lesson.get('export_dir',''))).write_pdf(target)
            elif kind == 'docx':
    ...
    def restore(self):
        ...
        self.store.data = json.loads(Path(source).read_text(encoding='utf-8')); self.store.save()
