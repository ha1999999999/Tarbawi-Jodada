...
class App(tk.Tk):
    ...
    def render_stage(self,parent):
        ...
        canvas=tk.Canvas(parent,bg='white',highlightthickness=0); scroll=ttk.Scrollbar(parent,orient='vertical',command=canvas.yview); inner=tk.Frame(canvas,bg='white'); inner.bind('<Configure>',lambda e:canvas.configure(scrollregion=canvas.bbox('all'))); canvas.create_window((0,0),window=inner,anchor='nw',width=parent.winfo_width() or 700); canvas.configure(yscrollcommand=scroll.set); canvas.pack(side='left',fill='both',expand=True); scroll.pack(side='right',fill='y')
        if s['kind']=='problem':
